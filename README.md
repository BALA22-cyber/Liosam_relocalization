A simple version of system that can relocalize in a built map is developed in this repository. The system is bsed on LIO_SAM.
...
1. Make sure the map should be saved in the right folder:
 check the config/params.yaml for the map location.
 alternatively it can be modified in src/globalLocalize.cpp

2. make sure to run 
```
chmod +x for scripts/odomtoworld.py
``` 
```
## Firstly, you need to run LIO-SAM, and then save the map in the default folder
```
2. Make sure to have the correct extrinsic parameters setup for the extrinsicROT and extrinsicRPY from  LIDAR->IMU in /config/params.yaml
...
/lio_sam/mapping/path publishes the location of the robot after initialization is succeed.

/lio_sam/mapping/poseOdomtomap publishes the initial location of the robot after initialization is succeed.

/transformed_pose continously publishes the location of the robot with respect to the map frame.
