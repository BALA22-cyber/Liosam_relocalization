...
public:
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        while (!imuQueOpt.empty())
        {
            ...
            if (imuTime < currentCorrectionTime - delta_t)
            {
                ...
                double dt = (lastImuT_opt < 0 || (imuTime - lastImuT_opt) <= 0) ? (1.0 / 200) : (imuTime - lastImuT_opt);
        ...
        if (!imuQueImu.empty())
        {
            ...
            for (int i = 0; i < (int)imuQueImu.size(); ++i)
            {
                ...
                double dt = (lastImuQT < 0 || (imuTime - lastImuQT) <= 0) ? (1.0 / 200) : (imuTime - lastImuQT);
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        double dt = (lastImuT_imu < 0 || (imuTime - lastImuT_imu) <= 0) ? (1.0 / 200) : (imuTime - lastImuT_imu);
